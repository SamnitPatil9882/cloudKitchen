...
func InitializeDatabase() (*sql.DB, error) {
	database, err := sql.Open("sqlite3", "../FOSDB.db")
	if err != nil {
	...
	query = "CREATE TABLE IF NOT EXISTS invoice (id INTEGER PRIMARY KEY AUTOINCREMENT,order_id INTEGER NOT NULL,date DATE NOT NULL,payment_method VARCHAR(100) NOT NULL,FOREIGN KEY (order_id) REFERENCES \"order\"(id))"
	statement, err = database.Prepare(query)
	if err != nil {
